...
%>
<span
	class="${cssClass} ${video.existCoverFile ? 'exist' : 'nonExist'}"
	onclick="fnImageView('${video.opus}')">${mode eq 's' ? 'C' : 'Cover'}</span>
<%
	} else if (view.equalsIgnoreCase("subtitles")) {
%>
<span
	class="${cssClass} ${video.existSubtitlesFileList ? 'exist' : 'nonExist'}"
	onclick="fnEditSubtitles('${video.opus}')">${mode eq 's' ? 's' : 'smi'}</span>
<%
	} else if (view.equalsIgnoreCase("overview")) {
%>
<span
	class="${cssClass} ${video.existOverview ? 'exist' : 'nonExist'}"
	onclick="fnEditOverview('${video.opus}')" title="${video.overviewText}">${mode eq 's' ? 'O' : 'Overview'}</span>
<%
	} else if (view.equalsIgnoreCase("download")) {
%>
<span class="${cssClass}">Download : ${video.videoDate}</span>
<%
	} else if (view.equalsIgnoreCase("release")) {
%>
<span class="${cssClass}">Release : ${video.releaseDate}</span>
<%
	} else if (view.equalsIgnoreCase("actress")) {
%>
<c:forEach items="${video.actressList}" var="actress" varStatus="status">
	<span class="${cssClass}"
		onclick="fnSearch('${actress.name}')" title="${actress}">${actress.name}</span>
	<img src="<c:url value="/resources/magnify${status.count%2}.png"/>"
		onclick="fnViewActressDetail('${actress.name}')" width="12px">
</c:forEach>
<%
...
%>
<span class="${cssClass}"
	onclick="fnSearch('${video.studio.name}')" title="${video.studio}">${video.studio.name}</span>
<img src="<c:url value="/resources/link.png"/>"
	onclick="fnViewStudioDetail('${video.studio.name}')">
<%
...
%>
<span class="${cssClass} rangeLabel"
	title="${video.scoreDesc}">${video.score}</span>
<%
	} else if (view.equalsIgnoreCase("rank")) {
%>
<input type="range" id="Rank-${video.opus}" name="points" ${mode eq 's' ? 'style="width:70px;"' : ''}
	min="<s:eval expression="@prop['rank.minimum']"/>"
	max="<s:eval expression="@prop['rank.maximum']"/>"
	value="${video.rank}" onmouseup="fnRank('${video.opus}')"
