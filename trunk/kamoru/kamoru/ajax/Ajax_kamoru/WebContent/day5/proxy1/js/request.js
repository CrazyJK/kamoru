...
ajax.xhr.Request = function(param) {
  ...
  this.callbackFunction = param.callbackFunction;
  this.method           = param.method;
  if (param.callbackObject) {
    this.callbackObject = param.callbackObject;
  } else {
...
//서버의 응답 결과는 callback 으로 지정된 콜백함수를 호출한다.
  send : function () {
    ...
    this.httpRequest.onreadystatechange = function () {
      if (localThis.callbackObject == localThis) {
        //사용자가 정의한 콜백함수를 호출한다.
        localThis.callbackFunction(localThis.httpRequest);
      } else {
        localThis.callbackFunction.call(localThis.callbackObject, localThis.httpRequest);
      }
    }
