package com.kamoru.code.spring.jdbc;
...
//		SimpleDao dao = ac.getBean(SimpleDao.class);
		...
		assertThat((BigDecimal)mmap.get("id"), is(new BigDecimal(1)));
		assertThat((String)mmap.get("name"), is("Spring"));
		assertThat((BigDecimal)mmap.get("point"), is(new BigDecimal(3.5)));
...
//		}
	...
	public void simpleJdbcInsertWithSqlParamSource() {
		...
		assertThat((BigDecimal)list.get(0).get("id"), is(new BigDecimal(1)));
		assertThat((String)list.get(0).get("name"), is("Spring"));
		assertThat((BigDecimal)list.get(0).get("point"), is(new BigDecimal(10.5)));
		
		assertThat((BigDecimal)list.get(1).get("id"), is(new BigDecimal(2)));
		assertThat((String)list.get(1).get("name"), is("Jdbc"));
		assertThat((BigDecimal)list.get(1).get("point"), is(new BigDecimal(3.3)));
	}
